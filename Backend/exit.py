...
now = datetime.now()# Run headless (no display window)
...
logging.info(f"Initializing {CAMERA_NAME} camera...    entry.py:33 - exit.py:37")
...
def load_known_faces(known_faces_dir="known_faces"):
    ...
    logging.info("Loading known faces...    entry.py:54 - exit.py:58")
    ...
    logging.info(f"✅ Loaded {len(known_embeddings)} known embeddings.    entry.py:70 - exit.py:74")
...
def process_known_face(name, emp_id, frame, now, now_time, seen_known, CAMERA_NAME):
    ...
    if now_time - last_seen >= EXIT_COOLDOWN:
        ...
        logging.info(f"✅ {CAMERA_NAME} logged exit for {key}    entry.py:107 - exit.py:121")
    return seen_known
...
def process_unknown_face(face, frame, now, now_time, seen_unknown, unknown_cooldowns):
    ...
    if now_time - unknown_cooldowns.get(emb_key, 0) >= UNKNOWN_COOLDOWN:
        ...
        logging.info(f"📸 Unknown saved: {photo_name}    entry.py:139 - exit.py:153")
...
def main_loop():
    ...
    if not cap.isOpened():
        logging.error(f"❌ {CAMERA_NAME} camera not accessible.    entry.py:163 - exit.py:177")
        return
 
    logging.info(f"🎥 {CAMERA_NAME} camera running...    entry.py:166 - exit.py:180")
    ...
    while True:
        ...
        if not ret:
            logging.warning(f"⚠️ Reconnecting {CAMERA_NAME}...    entry.py:174 - exit.py:188")
            time.sleep(1)
            cap = cv2.VideoCapture(RTSP_URL)
            continue
